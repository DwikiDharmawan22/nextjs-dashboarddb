  ...
  const onSubmit = async (data: RegisterFormData) => {
    try {
      const response = await fetch('/api/register', {
        ...
        body: JSON.stringify({ ...data, captcha: captchaInput, serverCaptcha: captcha }),
      });

      if (response.ok) {
        toast.success('Registrasi Berhasil!', { theme: 'dark' });
        router.push('/auth/login'); // Redirect to login instead of /home
      } else {
        const { error } = await response.json();
        toast.error(error || 'Registrasi gagal!', { theme: 'dark' });
      }
    } catch (error) {
      toast.error('Terjadi kesalahan!', { theme: 'dark' });
    }
  };
        ...
        <AuthFormWrapper>
          <div className="bg-[#D29BC7] bg-opacity-90 p-8 rounded-[48px] shadow-lg w-full max-w-4xl mx-auto h-full">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 w-full">
              ...
              <div className="space-y-1">
                ...
                <div className="mt-1">
                  <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      className={`h-1.5 ${passwordStrength === 100 ? 'bg-green-500' : passwordStrength >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`}
              ...
              {/* Confirm Password */}
              <div className="space-y-1">
                <label htmlFor="confirmPassword" className={`${doppio_one.className} block text-sm font-medium text-[#A64D79]`}>
                  Konfirmasi Password
                </label>
                <div className="relative">
                  <input
              ...
              <div className="space-y-1">
                <div className="flex items-center space-x-3">
                  ...
                  <FaRedo className="text-pink-600 hover:text-pink-800 cursor-pointer" size={14} onClick={refreshCaptcha} />
                </div>
                <input
                  type="text"
                  value={captchaInput}
              ...
              <button
                type="submit"
                className={`${doppio_one.className} w-full font-semibold py-2 rounded-lg transition-colors duration-200 NOC bg-[#FFE1F9] hover:bg-pink-600 text-[#6A1E55]`}
              >
                Register
              </button>
