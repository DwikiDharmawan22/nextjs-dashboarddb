...
export default async function AboutPage() {
  ...
  // Membagi produk menjadi array of arrays dengan maksimum 4 produk per frame
  const productFrames = [];
  for (let i = 0; i < shopProducts.length; i += 4) {
    productFrames.push(shopProducts.slice(i, i + 4));
  }

  return (
    <div className="p-4 mx-auto max-w-screen-2xl">
      {productFrames.map((frameProducts, index) => (
        <div key={index} className="flex-1 bg-white p-8 rounded-lg shadow-lg mb-8">
          <div className="product-grid grid grid-cols-1 md:grid-cols-4 gap-8">
            {frameProducts.map((product: ShopProduct) => (
              <div
